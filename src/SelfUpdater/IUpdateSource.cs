...
/// The name is the publisher's contract — a GitHub asset file name or a manifest
/// dictionary key — and is never interpreted by the engine; the consumer selects
...
/// that produced it (a URL for the HTTP/GitHub sources, but it could equally be a
/// file path, blob id, or torrent magnet for some future source).
/// </summary>
public sealed record UpdateAsset(string Name, string Location, string? Sha256 = null, long? Size = null);

/// <summary>A release a source knows about, with every asset it ships.</summary>
/// <param name="Version">The release version, parsed from the source's tag/manifest.</param>
...
/// described (a JSON manifest, a GitHub release, a package feed, a LAN share, ...).
